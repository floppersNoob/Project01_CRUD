...
export default function ManageSections({ sections, showArchived }) {
    ...
    const handleCloseModal = () => {
        ...
        setEditingSection(null);
        setFormData({ name: '' });
        showInfoAlert('Cancelled', 'Section form has been cancelled.');
    };
    ...
    const validateForm = (data) => {
        ...
        if (data.name.trim().length < 2) {
            showErrorAlert('Validation Error', 'Section name must be at least 2 characters.');
            return false;
        }
        return true;
    };
    ...
    const handleSubmit = (e) => {
        ...
        setProcessingMessage(editingSection ? `Updating "${formData.name}"...` : `Creating "${formData.name}"...`);
        
        // Force a re-render check
        setTimeout(() => {
            // Force DPWH loading to show regardless of state
            if (!isProcessing) {
                setIsProcessing(true);
            }
        }, 100);
        ...
        // Validation passed, submitting to backend
        
        if (editingSection) {
            // Update existing section
            router.put(`/sections/${editingSection.id}`, formData, {
                onSuccess: (page) => {
                    setIsProcessing(false);
                    handleCloseModal();
                ...
                onError: (errors) => {
                    setIsProcessing(false);
                    showErrorAlert('Error', 'Failed to update section. Please try again.');
                },
                onFinish: () => {
                    setIsProcessing(false);
                }
            ...
            router.post('/sections', formData, {
                onSuccess: (page) => {
                    setIsProcessing(false);
                    handleCloseModal();
                    showSuccessAlert('Success!', 'Section created successfully!');
                    setTimeout(() => {
                ...
                onError: (errors) => {
                    setIsProcessing(false);
                    showErrorAlert('Error', 'Failed to create section. Please try again.');
                },
                onFinish: () => {
                    setIsProcessing(false);
                }
    ...
    const openEditModal = (section) => {
        ...
        setIsCreateModalOpen(true);
        showInfoAlert('Edit Section', `Editing "${section.name}" section details.`);
    };
    ...
    const archiveSection = async (section) => {
        const result = await showConfirmAlert(
            'Archive Section',
            `Are you sure you want to archive "${section.name}"?`,
            {
        ...
        if (result) {
            // Show loading effect
            setIsProcessing(true);
            setProcessingMessage(`Archiving "${section.name}"...`);
            
            router.post(`/sections/${section.id}/archive`, {}, {
                onSuccess: (page) => {
                    setIsProcessing(false);
                    showSuccessAlert('Success!', `Section "${section.name}" archived successfully!`, { timer: 3000 });
                    setTimeout(() => {
                        router.reload();
                    }, 3000);
                },
                onError: (errors) => {
                    setIsProcessing(false);
                    showErrorAlert('Error', 'Failed to archive section. Please try again.');
                },
                onFinish: () => {
                    setIsProcessing(false);
                }
    ...
    const restoreSection = async (section) => {
        const result = await showConfirmAlert(
            'Restore Section',
            `Are you sure you want to restore "${section.name}"?`,
            {
        ...
        if (result) {
            // Show loading effect
            setIsProcessing(true);
            setProcessingMessage(`Restoring "${section.name}"...`);
            
            router.post(`/sections/${section.id}/restore`, {}, {
                onSuccess: (page) => {
                    setIsProcessing(false);
                    showSuccessAlert('Success!', `Section "${section.name}" restored successfully!`, { timer: 3000 });
                    setTimeout(() => {
                        router.reload();
                    }, 3000);
                },
                onError: (errors) => {
                    setIsProcessing(false);
                    showErrorAlert('Error', 'Failed to restore section. Please try again.');
                },
                onFinish: () => {
                    setIsProcessing(false);
                }
    ...
    const deleteSection = async (section) => {
        ...
        if (result) {
            // Show loading effect
            setIsProcessing(true);
            setProcessingMessage(`Deleting "${section.name}"...`);
            
            router.delete(`/sections/${section.id}`, {
                onSuccess: (page) => {
                    setIsProcessing(false);
                    showSuccessAlert('Success!', `Section "${section.name}" deleted successfully!`, { timer: 3000 });
                    setTimeout(() => {
                        router.reload();
                    }, 3000);
                },
                onError: (errors) => {
                    setIsProcessing(false);
                    showErrorAlert('Error', 'Failed to delete section. Please try again.');
                },
                onFinish: () => {
                    setIsProcessing(false);
                }
            ...
            <div className="space-y-6">
                ...
                <div className="mb-6 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">Manage organizational sections</p>
                    </div>
                    <button
                ...
                <div className="mb-6 flex justify-start">
                    <div className="flex space-x-1">
                        ...
                        <button
                            onClick={() => router.visit('/sections?archived=true')}
                            className={`px-4 py-2 text-sm font-medium transition-all duration-300 ease-in-out transform ${
                                showArchived
                                    ? 'bg-blue-600 text-white rounded-full shadow-md scale-105'
                                    : 'text-blue-600 hover:text-blue-800 hover:scale-105'
                ...
                <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-200">
                        <h3 className="text-lg font-semibold text-gray-900">
                            {showArchived ? 'Archived Sections' : 'Active Sections'}
                        </h3>
                    </div>
